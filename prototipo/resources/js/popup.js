...
export default class Popup extends Dialog {
  ...
  constructor({
    ...
    document.querySelector(`#${this.id}`).insertAdjacentHTML(
      'afterbegin',
      `<header id="title-${this.id}" class="justify-between flex">
          ${title}
          <button id="_close" class="bg-[#E6E6E6] text-base m-0 rounded-[5px] border-[1.5px] border-solid border-[#b6b6b6] hover:text-[rgb(202,0,0)] hover:border-[1.5px] hover:border-solid hover:border-[rgb(202,0,0)]">✖</button>
        </header>
        <hr class="my-3">`
    )
  ...
  addButtons() {
    ...
    if (this.#buttons.length) {
      ...
      document.querySelector(`#${this.id}`).insertAdjacentHTML('beforeend', `<hr class="my-3 text-[#ececec] border border-solid border-[#ececec]"><footer class = "flex justify-end"></footer>`)
